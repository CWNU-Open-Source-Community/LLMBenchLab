...
## 2026-08-30 observational Token overdraw 修复（`in_progress`）
...
- 本地验证已完成：backend `946 passed, 33 skipped`，真实 PostgreSQL+Redis integration `33 passed`，双方言 migration upgrade/downgrade/upgrade/check、`make lint`、frontend `39 passed`/build、Mock smoke `1 passed` 与 Compose config 均通过。当前个人 SQLite 已到 `0007`；四层 scope `overdrawn` 从 4 降为 0，7 Responses/7 ledger、407 input/599 output、13 张业务表行数均保留，`quick_check=ok`、FK=0。未调用真实 Provider。commit/push 和 exact-SHA CI 尚待完成，因此本维护仍为 `in_progress`。
...
- observational Token overdraw 修复：状态为 `in_progress`；目标 head `0007`、本地完整验证和当前库迁移已完成，仅 commit/push 与 exact-SHA CI 远程门禁待完成。
...
[NEXT_TASK.md](NEXT_TASK.md) 提供后续任务入口。observational Token overdraw 修复的本地验证和个人 SQLite 受控迁移已完成，当前只剩 commit/push 与 exact-SHA CI；仓库级闭环后才恢复 P2-07 最小只读 recovery verifier。P2-06 已完成仓库级收尾，P2-07 仍为 `planned`；Phase 2 与 Phase 3 继续保持 `in_progress`。
