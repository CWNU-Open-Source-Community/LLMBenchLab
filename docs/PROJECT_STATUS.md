...
## 2026-08-29 数据库兼容修复（`local_verified`）
...
- 本地 migration `52 passed`；最终完整 `make test` 为后端 `927 passed, 33 skipped`、前端 `38 passed`，`make lint`、Mock smoke 和 Compose config 均通过。精确 SHA 远程 CI 尚未执行，因此本维护任务暂不标记 completed，Phase 2/P2-07 状态不变。
...
| 2026-08-29 DB compatibility repair | migration `52 passed`；最终完整 backend `927 passed, 33 skipped`、frontend `38 passed`；lint/smoke/config、真实失败备份副本与当前库 startup/check 全绿 | current head `20260829_0006`；本地已验证，exact-SHA CI 待执行 |
