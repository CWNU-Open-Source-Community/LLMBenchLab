...
- 小型评测集本地加载：状态为 `completed`；五套 100 题 mini 子集与完整 100 题 XCOPA validation 已生成并导入，Loader/Evaluator、API/数据库/Hash 与完整性验证通过；本导入任务没有创建或取消 Run，后续并发 Run 操作另行留有审计时间线。
...
| 2026-08-30 小型评测集本地加载 | 六个 ZIP Loader 校验、重复生成与 archive/Dataset Hash 对账通过；API 导入 `201` × 6；金标评分器格式自检 600/600（非模型成绩）；目标测试 `40 passed`；数据库为 `10` Benchmarks、`24,877` Questions，`quick_check=ok`、FK `0`、head `0007` | 六套均为 100 题且可直接选择；导入任务未创建/取消 Run，既有大 Run 与随后创建的 MGSM mini Run 的 Response/Provider 流量属于并发客户端操作，不归因于导入 |
