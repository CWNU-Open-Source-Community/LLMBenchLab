...
正式 wrapper `make phase2-slo` 在 capacity child 之上实现 [ADR-0012](decisions/ADR-0012-single-host-slo-capacity-qualification.md) 的 `P2-local-control-plane-v1`。它只接受 exact clean commit，固定单主机的 PostgreSQL 16、Redis 7、一个 API、两个 Worker、pool/overflow `5/5`、retry `max/base/cap=3/1/30s` 和 `lease/heartbeat/poll=30/10/1s`，串行保留 1 次 warm-up 与 5 次 measured trial。wrapper 再次验证每轮拓扑、Settings read-back、环境/数据/配置指纹、三 cell、fault/fairness、ledger projection 和 cleanup；吞吐按 `completed_questions / wall_duration_seconds` 重算，双/单 Worker scale 以同 trial 配对，避免信任 child 汇总或跨轮挑样。

aggregate 与 raw child evidence 都位于 Git 忽略目录；aggregate 只含 commit/source hash、稳定指纹、child 相对路径/hash、脱敏统计/判定、容量模型和 cleanup 摘要的 allowlist，不复制 stdout/log、DSN/URL、环境变量、题目、Prompt/Response 或秘密材料。child 使用独立进程组，超时/中断后给 scoped cleanup 420 秒。该架构把正确性硬门禁、统计资格和证据最小化分层，但命令存在不等于已经取得正式多轮合格证据；任何结果也只描述被记录的硬件、配置和 commit，不是 Provider 性能、生产 SLO/SLA、HA 或无限扩展证明。GitHub-hosted CI 只验证 validator、统计和失败路径，不承担绝对性能门禁。基线解释见 [`PERFORMANCE.md`](./PERFORMANCE.md)，扩缩容、预算/背压、settlement unknown、DB/Redis/lease 恢复与 0004 安全回滚见 [`OPERATIONS.md`](./OPERATIONS.md)。
