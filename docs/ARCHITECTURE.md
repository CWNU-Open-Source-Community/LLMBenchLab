...
隔离容量入口 `make phase2-capacity` 的当前代码以真实 PostgreSQL 16、Redis 7、1/2 Worker 和纯 Mock workload 比较吞吐/延迟，显式 PUT/读回全有限 policy，并要求并发 backlog 准确分裂为 `202/429`、`question_quantum<15` 产生 cooperative yield，以及低流量 Mock Model 在高流量 backlog 排空前获得 slice；另注入 lease、Redis、重复通知和 ledger/audit 对账场景。脱敏 JSON 记录环境、数据、并发、p50/p95/p99、错误/重试及 DB/queue 压力。仓库现有真实 artifact 来自这些增强之前的 dirty worktree；当前候选尚未完整重跑新 evidence，因而不能宣称上述新场景已在真实 Compose 通过。任何 evidence 也只描述被记录的硬件、配置和 dirty worktree/commit，不是 Provider 性能、生产 SLO/SLA 或无限扩展证明。基线解释见 [`PERFORMANCE.md`](./PERFORMANCE.md)，扩缩容、预算/背压、settlement unknown、DB/Redis/lease 恢复与 0004 安全回滚见 [`OPERATIONS.md`](./OPERATIONS.md)。
