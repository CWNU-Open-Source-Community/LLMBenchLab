...
- 单个 Run 的输出/等待配置：Web 可按 Benchmark 建议选择 `max_tokens=1..131072`，或用 `null` 省略上游字段；`read_timeout_seconds=1..1800` 随 execution snapshot 固化。未显式提供 `max_tokens` 的通用 API/protocol-v1 默认仍为 256；这些每题参数不等同于 P2-05 全局预算治理。
...
| 长推理被截断或读超时 | Benchmark 建议值、1..131072/Provider 默认、1..1800 秒快照、`output_truncated` 证据 | Provider 能力/默认值各异；没有全局 Token/费用硬预算 |
