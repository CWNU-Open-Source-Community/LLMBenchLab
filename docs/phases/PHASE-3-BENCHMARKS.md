...
沙箱范围未开始。该当前工作树 UI 切片的完整本地门禁已通过，commit/push 与精确 SHA CI 查询尚待完成。
...
当前工作树切片。IFEval、通用 Plugin SDK、代码题/沙箱、分组/子集完整 UI 与红队验收仍缺失，且
本轮远程交付尚待完成，且其余范围仍缺失，故本阶段不得标为 `completed`。
